...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(
        ...
        if use_bias:
            ...
            self.cls = list([nn.Parameter(torch.randn(c, head_channels, 1, 1).cuda()) for c in classes])
            for i, cls in enumerate(self.cls):
                nn.init.kaiming_normal_(cls, mode='fan_in')
                self.register_parameter('cls_{}'.format(i), cls)
        ...
        if use_cosine:
            self.proto = list([torch.zeros(head_channels, 1, 1, 1) for c in range(self.tot_classes)])
        ...
        if only_bg:
            ...
            for i, mod in enumerate(self.cls):
                if i == 0 and self.multi_modal_background:
                    ...
                    w = F.normalize(mod, dim=1, p=2)
                    out.append(F.conv2d(x_pl, w))
                    # if labels is not None:
                    #     labels_ = F.interpolate(labels.float().unsqueeze(1), (x_pl.shape[2], x_pl.shape[3]),mode='bilinear').squeeze(1)
                else:
                    out.append(mod(x_pl))
    ...
    def forward(self, x, labels=None, scales=None, do_flip=False, ret_intermediate=False, only_bg=False):
        out_size = x.shape[-2:]
        ...
        # print(sem_logits.max(), sem_logits.min())
