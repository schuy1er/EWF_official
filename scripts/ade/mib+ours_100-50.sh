...
NAME=MiB+ours
METHOD=FT
OPTIONS="--checkpoint checkpoints/step/ --loss_kd 10 --unce --unkd --init_balanced --weight_fusion --fix_pre_cls"
...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size 12 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs 60 --method ${METHOD} --opt_level O1 ${OPTIONS}
