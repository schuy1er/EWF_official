...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y): ### feats(bs,k,channel), labels(bs,m), predict(bs,m)
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:
                hard_indices = ((this_y_hat == cls_id) & (this_y != cls_id)).nonzero()  #### belong to class cls_id but predict not in that
                easy_indices = ((this_y_hat == cls_id) & (this_y == cls_id)).nonzero()  #### belong to & predict in that

                num_hard = hard_indices.shape[0] #### calculate num of two kinds
                num_easy = easy_indices.shape[0]

                if num_hard >= n_view / 2 and num_easy >= n_view / 2:  ### to keep several
