...
from utils.loss import (NCA, BCESigmoid, BCEWithLogitsLossWithIgnoreIndex,
                        ...
                        UnbiasedKnowledgeDistillationLoss, UnbiasedNCA, PixelContrastLoss,
                        soft_crossentropy)
...
class Trainer:
    ...
    def __init__(self, model, model_old, proj_head, device, opts, trainer_state=None, classes=None, step=0):
        ...
        self.momentum_proto_cls = opts.momentum_proto_cls
        self.cls_contrastive = opts.cls_contrastive
        
        if classes is not None:
            self.iter_num = list([0 for i in range(classes[-1])])
        ...
        #EMA
        self.model_last_iter_dict = None
        self.iter = 1

    def before(self, train_loader, logger):
        self.model_last_iter_dict = self.model.state_dict()
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        cls_contrast_loss = torch.tensor(0.)

        cls_contrast_loss = cls_contrast_loss.to(device)
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            optim.zero_grad()
            outputs, features = model(images, labels=labels, ret_intermediate=self.ret_intermediate)
            
            # if torch.isnan(outputs).sum() > 0:
            #     print("an nan number appear in outputs")
            ...
            loss_tot = loss + lkd + lde + l_icarl + pod_loss + loss_entmin + cls_contrast_loss
