...
def run_step(opts, world_size, rank, device):
    ...
    if opts.lr_old is not None and opts.step > 0:
        ...
        if opts.cosine is True:
            params.append(
                {
                    "params": filter(lambda p: p.requires_grad, model.cls),
                    'weight_decay': opts.weight_decay
                }
            )
        else:
    ...
    ### xjw changed(for the weight fusion strategy) ####
    if opts.weight_fusion is True:
        if rank == 0 and opts.step > 0 and TRAIN:
            if opts.dataset == "voc":
                ckpt_old = f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step - 1}.pth"
            elif opts.dataset == "ade":
                ckpt_old = f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step - 1}.pth"
            ...
            logger.info("merge_weight is: {}".format(merge_weight))
            
            if opts.threshold_choice is True: #### you can choose to use this technique to further improve the performance, which I didn't mention in paper.
                pick = torch.zeros(0)
                for name, weight in checkpoint_old["model_state"].items():
                    magnitude = abs(weight - checkpoint["model_state"][name])
                    pick = torch.cat((pick, magnitude.flatten()), dim=0)
                
                merge_threshold = torch.topk(pick, np.int(pick.shape[0]*0.45))[0][-1]
                print("merge_threshold is: {}".format(merge_threshold))
                # merge_threshold = 3e-4
                for name, weight in checkpoint_old["model_state"].items():
                    checkpoint["model_state"][name] = torch.where(
                    abs(weight - checkpoint["model_state"][name]) < merge_threshold,
                    checkpoint["model_state"][name],
                    (1 - merge_weight) * weight + merge_weight * checkpoint["model_state"][name]
                    )
            else:
                for name, weight in checkpoint_old["model_state"].items():
                    checkpoint["model_state"][name] = (1 - merge_weight) * weight + merge_weight * checkpoint["model_state"][name]

            torch.save(checkpoint, f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step}.pth")
    ...
    if True: #TRAIN:
        ...
        del checkpoint
        trainer = Trainer(model, None, None, device=device, opts=opts, step=opts.step)
    ...
    logger.info(val_metrics.to_str(val_score))
    # logger.info(val_score['TN'])
    # logger.info(val_score['FP'])
